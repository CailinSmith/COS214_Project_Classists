...
Product* Receipt::removeProduct(Product* product) {
    for (auto it = orderPlants.begin(); it != orderPlants.end(); ++it) {
        if (*it == product) {
            orderPlants.erase(it);
            regenerateReceiptContent();  
            return product; 
        }
    }
    return nullptr; 
...
void Receipt::regenerateReceiptContent() {
    std::stringstream receipt;
